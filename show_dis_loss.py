...
if __name__ == '__main__':
    feature_map = torch.zeros((1, 1, 20, 20))
    ...
    # 将指定行的值设置为1
    feature_map[:, :, line_row_index1, :] = 1
    feature_map[:, :, line_row_index2, :] = 1

    label = torch.zeros((1, 20, 20))

    # 定义要画横线的行索引
    line_row_index_label = 12

    # 将指定行的值设置为1
    label[:, line_row_index_label, :] = 1

    dis = Label_Distance_Loss()
    distance = dis(feature_map, label)
    """
    (12 - 10) - 1 = 1
    (12 - 5) - 1 = 6
    (1 + 6) / 2 = 3.5
    ...
    # 可选：将张量转换为NumPy数组，并使用Matplotlib进行可视化
    circle_numpy1 = feature_map.numpy()
    plt.imshow(circle_numpy1[0, 0], cmap='gray')
    plt.axis('off')
    plt.show()
    label = label.unsqueeze(dim=1)
    circle_numpy2 = label.numpy()
    plt.imshow(circle_numpy2[0, 0], cmap='gray')
    plt.axis('off')
    plt.show()
